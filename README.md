...
```shell
git clone https://github.com/aobao32/kiro-support-feishu-bot-agentscope-mimo.git
cd kiro-support-feishu-bot-agentscope-mimo
```
...
本文第四节用 `export` 设置的环境变量只属于当前登录 shell，关闭终端或退出登录后后台进程便读不到。为让机器人脱离终端长期稳定运行，本项目通过 `.env` 文件管理敏感配置，程序启动时由 `config.py` 自动加载，与 shell 解耦。
...
`.env` 内容如下（已被 `.gitignore` 忽略，不会提交到仓库）：
...
`config.py` 在读取环境变量前会调用 `load_dotenv()` 加载该文件。已存在于真实环境中的同名变量优先，不会被 `.env` 覆盖，因此第四节的 `export` 方式仍然有效，两者可共存。

### 2、后台启动

配置好 `.env` 后，无需再 `export`，直接用 `nohup` 将进程放到后台并把日志重定向到文件：

```shell
nohup uv run python main.py > logs/nohup.out 2>&1 &
```
...
也可使用 `tmux` 或 `screen` 保持会话，效果相同。

### 3、开机自启（可选，Ubuntu 24.04 systemd）

如需服务器重启后自动拉起，可用 systemd 把机器人注册为系统服务。新建 `/etc/systemd/system/kiro-bot.service`，内容如下：

```shell
[Unit]
Description=Kiro Feishu Support Bot (AgentScope + MiMo)
...
Type=simple
User=ubuntu
WorkingDirectory=/home/ubuntu/kiro-support-feishu-bot-agentscope-mimo
# .env 由 config.py 自动加载，无需在此重复声明环境变量
...
- `User`、`WorkingDirectory` 改成实际部署用户和项目绝对路径。
- `ExecStart` 中 `uv` 要用绝对路径（`which uv` 查询，通过官方脚本安装时通常在 `~/.local/bin/uv`），因为 systemd 不加载登录 shell 的 PATH。
- 环境变量沿用方案一的 `.env`，由程序自动加载；若更想交给 systemd 管理，也可改用 `EnvironmentFile=/home/ubuntu/kiro-support-feishu-bot-agentscope-mimo/.env` 指令注入。

启用并启动服务：

```shell
sudo systemctl daemon-reload
sudo systemctl enable --now kiro-bot.service

# 查看状态与日志
systemctl status kiro-bot.service
journalctl -u kiro-bot.service -f
```

`Restart=always` 会在进程异常退出时自动重启，`enable` 实现开机自启。
